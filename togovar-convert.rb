...
require 'builder'
require 'open3'
require './lib/togovar-method.rb'
require './lib/togovar-config.rb'
#sin require '/usr/local/bin/lib/togovar-method.rb'
#sin require '/usr/local/bin/lib/togovar-config.rb'
...
OptionParser.new{|opt|
	...
	opt.on('-r', 'previous excel sheet names'){|v|
		previous_f = true
		puts "Use previous excel sheet names without TogoVar"
	}
...
### Read the TogoVar-repository submission excel file
...
if ARGV[0]
	...
	raise "No TogoVar-repository metadata excel to open."
end
...
unless study_h[:vload_id].empty?
	...
	error_ignore_sv_a.push(["JV_SV0003", "vload_id is missing. TogoVar-repository will fill in this value."])
end
...
xml_f.puts xml.SUBMISSION(submission_attr_h){|submission|
	...
	for experiment in experiment_a
		...
		if !experiment[:"Method Platform"].empty?
			...
			if experiment[:"Method Platform"].scan(/(#{xref_db_all_regex}) *: *([-A-Za-z0-9]+)/).size > 0
				...
				experiment_e.PLATFORM(:platform_name => experiment[:"Method Platform"]){|platform_e|
	...
	for vc_input, variant_call_a in total_variant_call_h
		...
		for variant_call in variant_call_a
			...
			submission.VARIANT_CALL(variant_call_attr_h){|variant_call_e|
				...
				# DESCRIPTION
				variant_call_e.DESCRIPTION(variant_call[:Description]) if variant_call[:Description] && !variant_call[:Description].empty?
	...
	if variant_region_a.empty? && !variant_call_a.empty?
		...
		warning_sv_a.push(["JV_VCFS0007", "Variant regions are not submitted. TogoVar-repository will generate variant regions from variant calls."])
		...
		# 2. complex と translocation は mutation id でまとめる。same pos and type ではまとめない。variant call = 1 - region type = translocation, calls > 1 - region type = complex
		# 3. translocation with insertion は mutation id でまとめる。same pos and type ではまとめない。complex
		...
		for variant_call in variant_call_a
			...
			for r_mutation_id, r_calls_by_mutation_id_a in variant_calls_by_mutation_id_h
				if r_calls_by_mutation_id_a.include?(r_variant_call_id)
					...
					if variant_call[:"Comment"] == "ins_with_trans"
						...
						r_assertion_method = "Overlapping breakpoint and insertion calls merged into regions by TogoVar-repository staff (no regions submitted)"
					elsif variant_call[:"Comment"] == "ins_with_sbnd"
							r_variant_region_type = "complex chromosomal rearrangement"
							r_assertion_method = "Overlapping single breakend and insertion calls merged into regions by TogoVar-repository staff (no regions submitted)"
					else
						if supporting_variant_call_id_a.size == 1
							r_variant_region_type = "translocation"
							r_assertion_method = "Breakpoint calls grouped by submitter, and merged into regions by TogoVar-repository staff (no regions submitted)"
					...
					# merge された call を除くため格納, 自分自身の call id 以外の ids
			...
			# insertion with translocation は identity で merge しない
			...
			unless group_by_mutation_id_f # Mutation id によるグループ化を優先。translocation 以外の mutation ID は無いはずなので実質的に無効
				for hash_key, r_calls_by_identity_a in identical_variant_call_site_id_h
					if r_calls_by_identity_a.include?(r_variant_call_id)
						...
						r_assertion_method = "Identical calls merged by TogoVar-repository staff (no regions submitted)"
		...
		submission.VARIANT_REGION(variant_region_attr_h){|variant_region_e|
			...
			if variant_region_type == "complex chromosomal rearrangement" || variant_region_type == "translocation"
				...
				supporting_variant_call_a.each{|supporting_variant_call_h|
					if !supporting_variant_call_h[:"Mutation ID"].nil? && !supporting_variant_call_h[:"Mutation ID"].empty? && (supporting_variant_call_h[:"Mutation Order"].nil? || !supporting_variant_call_h[:"Mutation Order"].is_a?(Integer))
						...
						variant_region_tsv_log_a.push("#{variant_region[:row].join("\t")}\t# JV_SV0066 Error: Variant regions with type 'complex chromosomal rearrangement' and 'translocation' must have mutation order in their supporting variant calls with Mutation ID. TogoVar-repository will embed order(s)")
					end
				} # supporting_variant_call_a
			...
			if mutation_id_exist
				...
				supporting_variant_call_a.each{|supporting_variant_call_h|
					...
					# supporting call が translocation の場合 FROM TO を PLACEMENT に出力
					if supporting_variant_call_h[:"Variant Call Type"].match?(/translocation/)
						...
						# mutataion molecule がある場合に mutation order を出力
						if supporting_variant_call_h[:"Mutation Molecule"] && !supporting_variant_call_h[:"Mutation Molecule"].empty?
	...
	## JV_SV0068: Missing assertion_method
	error_ignore_sv_a.push(["JV_SV0068", "Region MUST have an assertion_method. TogoVar-repository will fill in this field. Variant Region: #{missing_assertion_method_region_a.size} sites, #{missing_assertion_method_region_a.size > 4? missing_assertion_method_region_a[0, limit_for_etc].join(",") + " etc" : missing_assertion_method_region_a.join(",")}"]) unless missing_assertion_method_region_a.empty?
